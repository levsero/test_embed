describe('embed.chat', () => {
  ...
  const trackZopimApisSpy = jasmine.createSpy('trackZopimApis')
  ...
  beforeEach(() => {
    ...
    mockZopim.livechat = {
      ...
      button: jasmine.createSpyObj('button', ['show'])
    }
    ...
    mockRegistry = initMockRegistry({
      ...
      'service/api/zopimApi/helpers': {
        ...
        trackZopimApis: trackZopimApisSpy
      }
    })
  ...
  describe('create', () => {
    it('should create and add a chat', () => {
      expect(_.keys(chat.list()).length).toBe(0)

      chat.create('dave')

      expect(_.keys(chat.list()).length).toBe(1)
    })
    ...
    it('should store the zopimId in config', () => {
      ...
      expect(chat.get(chatName).config.zopimId).toEqual(zopimId)
    })
  })
  ...
  describe('setUser', () => {
    ...
    it('calls livechat.setName and livechat.setUser', () => {
      ...
      expect(mockZopim.livechat.setName).toHaveBeenCalledWith('david')
      expect(mockZopim.livechat.setEmail).toHaveBeenCalledWith('david@email.com')
    })
    ...
    it('does NOT call livechat.setName when user.name is missing', () => {
      ...
      expect(mockZopim.livechat.setEmail).toHaveBeenCalledWith('daniel@email.com')
      expect(mockZopim.livechat.setName).not.toHaveBeenCalled()
    })
  })
  ...
  describe('render', () => {
    ...
    it('should inject the zopim bootstrap script into the document', () => {
      ...
      expect(document.querySelectorAll('body > script').length).toEqual(1)

      const snippetText = document.querySelectorAll('body > script')[0].innerHTML

      expect(snippetText.indexOf(zopimId) !== -1).toBe(true)

      expect(snippetText.indexOf('https:') !== -1).toBe(true)
    })
    ...
    it('should call zopim.livechat.mobileNotifications.setIgnoreChatButtonVisibility()', () => {
      ...
      expect(
        mockZopim.livechat.mobileNotifications.setIgnoreChatButtonVisibility
      ).toHaveBeenCalled()
    })

    it('queues up zopim tracking', () => {
      mockZopim._setByWW = true
      mockZopim._ = []
      chat.create(chatName, { zopimId: zopimId }, mockStore)
      chat.render(chatName)

      expect(mockZopim._.length).toEqual(1)
      mockZopim._[0]()
      expect(trackZopimApisSpy).toHaveBeenCalled()
    })
    ...
    describe('brands', () => {
      ...
      describe('when more than one brand exists', () => {
        ...
        it('adds the brand name as a chat tag', () => {
          expect(mockZopim.livechat.addTags).toHaveBeenCalledWith(['ACME'])
        })
      })
      ...
      describe('when no more than one brand exists', () => {
        ...
        it('does not add the brand name as a chat tag', () => {
          expect(mockZopim.livechat.addTags).not.toHaveBeenCalled()
        })
      })
    })
    ...
    describe('zopim.endpoint', () => {
      describe('when the config does not exist', () => {
        ...
        it('should inject v2.zopim.com into the zopim snippet', () => {
          ...
          expect(scriptTag.innerHTML.indexOf('v2.zopim.com')).not.toEqual(-1)
        })
      })
      ...
      describe('when the config exists', () => {
        ...
        it(`should inject ${endpoint} into the zopim snippet`, () => {
          ...
          expect(scriptTag.innerHTML.indexOf(endpoint)).not.toEqual(-1)
        })
      })
    })

    describe('mediator broadcasts', () => {
      let mockZopim, onHideCall, onStatusCall, onUnreadMsgsCall, onChatStartCall, onChatEndCall
      ...
      describe('livechat.onHide', () => {
        it('should broadcast <name>.onHide', () => {
          ...
          expect(mockMediator.channel.broadcast).toHaveBeenCalledWith('dave.onHide')
        })
      })
      ...
      describe('zopim.livechat.onStatus', () => {
        ...
        describe('when agent is online', () => {
          ...
          it('broadcasts <name>.onOnline', () => {
            expect(mockMediator.channel.broadcast).toHaveBeenCalledWith('dave.onOnline')
          })
        })
        ...
        describe('when agent is offline', () => {
          ...
          it('broadcasts <name>.onOffline', () => {
            expect(mockMediator.channel.broadcast).toHaveBeenCalledWith('dave.onOffline')
          })
        })
        ...
        it('calls updateZopimChatStatus with the status', () => {
          ...
          expect(updateZopimChatStatusSpy).toHaveBeenCalledWith('online')
        })
      })
      ...
      describe('zopim.onUnreadMsgs', () => {
        it('should broadcast <name>.onUnreadMsgs regardless of count', () => {
          ...
          expect(mockMediator.channel.broadcast).toHaveBeenCalledWith('dave.onUnreadMsgs', count)
      ...
      describe('zopim.onChatStart', () => {
        ...
        it('broadcasts <name>.onChatStart', () => {
          expect(mockMediator.channel.broadcast).toHaveBeenCalledWith('dave.onChatStart')
        })
      })
      ...
      describe('zopim.onChatEnd', () => {
        ...
        it('broadcasts <name>.onChatEnd', () => {
          expect(mockMediator.channel.broadcast).toHaveBeenCalledWith('dave.onChatEnd')
        })

        it('calls zopimEndChat', () => {
          expect(zopimEndChatSpy).toHaveBeenCalled()
        })
      })
      ...
      describe('zopim.onError', () => {
        it('should broadcast <name>.onError', () => {
          ...
          expect(mockMediator.channel.broadcast).toHaveBeenCalledWith('dave.onError')
        })
      })
    })
    ...
    describe('mediator subscriptions', () => {
      ...
      describe('<name>.refreshLocale', () => {
        ...
        it('calls zopim.livechat.setLanguage', () => {
          ...
          expect(mockZopim.livechat.setLanguage).toHaveBeenCalled()
        })
      })
      ...
      describe('<name>.show', () => {
        it('should call zopim.button.show()', () => {
          ...
          expect(mockZopim.livechat.button.show).toHaveBeenCalled()
        })

        it('should call zopim.livechat.mobileNotifications.setDisabled(false)', () => {
          pluckSubscribeCall(mockMediator, 'dave.show')()

          expect(mockZopim.livechat.mobileNotifications.setDisabled).toHaveBeenCalledWith(false)
        })
      })
      ...
      describe('<name>.hide', () => {
        it('should call zopim.livechat.hideAll()', () => {
          ...
          expect(mockZopim.livechat.hideAll).toHaveBeenCalled()
        })

        it('should call zopim.livechat.mobileNotifications.setDisabled(true)', () => {
          pluckSubscribeCall(mockMediator, 'dave.hide')()

          expect(mockZopim.livechat.mobileNotifications.setDisabled).toHaveBeenCalledWith(true)
        })
      })
    ...
    describe('init', () => {
      ...
      describe('setPosition', () => {
        describe('when vertical position is top', () => {
          it('should set the vertical position', () => {
            ...
            expect(mockZopim.livechat.window.setPosition).toHaveBeenCalledWith('tr')
          })
        })
        ...
        describe('when horizontal position setting is left', () => {
          it('should set the horizontal position', () => {
            ...
            expect(mockZopim.livechat.window.setPosition).toHaveBeenCalledWith('bl')
          })
        })
      })
      ...
      describe('setOffsetVertical', () => {
        it('should set the vertical offset', () => {
          ...
          expect(mockZopim.livechat.window.setOffsetVertical).toHaveBeenCalledWith(10)
        })
      })
      ...
      describe('when horizontal position setting is left', () => {
        it('should set the horizontal offset', () => {
          ...
          expect(mockZopim.livechat.window.setOffsetHorizontal).toHaveBeenCalledWith(28)
        })
      })
    })
  })
