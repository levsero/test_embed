describe('formatter submitTicket helper', () => {
  ...
  describe('formatRequestData', () => {
    ...
    it('formats the requester correctly', () => {
      expect(params.request.requester).toEqual(
    ...
    it('Adds the correct tag', () => {
      expect(params.request.tags).toContain('web_widget')
    })
    ...
    it('adds any extra tags', () => {
      ...
      expect(params.request.tags).toContain('extra_tag')
    })

    it('Adds the correct via_id', () => {
      /* eslint camelcase:0 */
      expect(params.request.via_id).toEqual(48)
    })
    ...
    describe('when name field is empty', () => {
      ...
      const assertEmailFormat = (email, expected) => {
        it(`formats the name correct based on the email ${email}`, () => {
          ...
          expect(params.request.requester.name).toEqual(expected)
        })
      }
    ...
    it('uses the description as the subject', () => {
      expect(params.request.subject).toEqual(formParams.description)
    })
    ...
    it('trims the subject if it is too long', () => {
      ...
      expect(params.request.subject).toEqual(
        'this text is longer then 50 characters xxxxxxxxxxx...'
      )
    })
    ...
    describe('when stores referrerPolicy is false', () => {
      it('adds submitted from to the description', () => {
        ...
        expect(params.request.comment.body).toBe(
    ...
    describe('when referrerPolicy is true', () => {
      ...
      it('adds submitted from to the description', () => {
        ...
        expect(params.request.comment.body).toBe(formParams.description)
      })
    })
    ...
    describe('when the form has custom ticket fields', () => {
      ...
      it('should correctly format custom fields', () => {
        expect(params.request.fields['22660514']).toBe('mockCustomField')
      })
    })

    describe('when the form is a user ticket form', () => {
      let mockTicketForm, ticketFields
      ...
      it('should correctly format custom fields', () => {
        expect(params.request.fields[4]).toBe('Cheeseburger')
      })

      describe('when subject field is not available', () => {
        it('uses the description as the subject', () => {
          expect(params.request.subject).toBe('Just saying Hi')
        })
      })
      ...
      describe('when the subject field is available', () => {
        ...
        it('should correctly format the subject field', () => {
          expect(params.request.fields[2]).not.toBe('Hello')

          expect(params.request.subject).toBe('Hello')
        })
      })

      it('should correctly format the description field', () => {
        expect(params.request.fields[1]).not.toBe('Just saying Hi')

        expect(params.request.comment.body).toContain('Just saying Hi')
      })

      it('should send through the ticket_form_id', () => {
        expect(params.request.ticket_form_id).toBe(50)
      })
    })
  })
})
