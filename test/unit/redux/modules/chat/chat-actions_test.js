...
describe('chat redux actions', () => {
  ...
  describe('chatLogout', () => {
    ...
    describe('when authenticated', () => {
      ...
      it('calls endChat', () => {
        expect(mockEndChat).toHaveBeenCalledWith(jasmine.any(Function))
      })
      ...
      describe('Web SDK callback', () => {
        ...
        it('dispatches an action with type CHAT_USER_LOGGING_OUT', () => {
          expect(mockStore.getActions()[0].type).toEqual(actionTypes.CHAT_USER_LOGGING_OUT)
        })
        ...
        it('calls init with correct args', () => {
          expect(mockInit).toHaveBeenCalledWith(mockZChatConfig)
        })

        it('calls the "on" api with the correct args', () => {
          expect(mockOn).toHaveBeenCalledWith('connection_update', jasmine.any(Function))
        })
          ...
          describe('when connection status is connected', () => {
            ...
            describe('when user is logging out', () => {
              ...
              it('dispatches an action with type CHAT_USER_LOGGED_OUT', () => {
                expect(mockStore.getActions()[1].type).toEqual(actionTypes.CHAT_USER_LOGGED_OUT)
              })
            })
