describe('onWidgetOpen', () => {
  ...
  describe('when not on mobile', () => {
    ...
    it('does not apply any scroll hacks', () => {
      expect(setWindowScrollSpy).not.toHaveBeenCalled()
      expect(setScrollKillerSpy).not.toHaveBeenCalled()
      expect(revertWindowScrollSpy).not.toHaveBeenCalled()
    })
  })
    ...
    describe('when active embed is not zopimChat', () => {
      ...
      describe('when web widget goes from not visible to visble', () => {
        ...
        it('applies scroll hacks', () => {
          expect(setWindowScrollSpy).toHaveBeenCalledWith(0)
          expect(setScrollKillerSpy).toHaveBeenCalledWith(true)
        })
      })
      ...
      describe('when web widget already visible', () => {
        ...
        it('applies scroll hacks', () => {
          expect(setScrollKillerSpy).toHaveBeenCalledWith(false)
          expect(revertWindowScrollSpy).toHaveBeenCalled()
        })
      })
    })
  })
