describe('ImageMessage component', () => {
  let ImageMessage, component, sharedPropTypes, mockFile
  ...
  describe('shared functionality', () => {
    beforeEach(() => {
      ...
      component = domRender(
        <ImageMessage file={mockFile} onImageLoad={onImageLoadSpy} placeholderEl={placeholderEl} />
      )
    })
    ...
    describe('#onLoad', () => {
      ...
      it('calls onImageLoad prop', () => {
        expect(onImageLoadSpy).toHaveBeenCalled()
      })
    })
    ...
    describe('#render', () => {
      ...
      describe('when the image has loaded', () => {
        ...
        it('renders an anchor tag', () => {
          expect(result.props.children.props.href).toEqual(mockFile.url)
        })

        it('changes the background image', () => {
          expect(result.props.children.props.children.props.style).toEqual(
    ...
    describe('#render', () => {
      ...
      describe('when the image has not yet loaded', () => {
        ...
        it('renders the spinner', () => {
          expect(result.props.children.props.children.props.className).toContain('spinner')
        })
      })
      ...
      describe('when the image has loaded', () => {
        ...
        it('does not render the spinner', () => {
          expect(result.props.children.props.children.props.className).not.toContain('spinner')
        })
      })
    })
  })
