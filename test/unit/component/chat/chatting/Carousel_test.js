  ...
  describe('.render', () => {
    ...
    describe('Default Props', () => {
      ...
      it('should render PureCarousel', () => {
        expect(TestUtils.isElementOfType(result, PureCarousel)).toEqual(true)
      })

      it('should render the correct number of children within PureCarousel', () => {
        expect(result.props.children.length).toEqual(mockProps.items.length)
      })

      it('should render StructuredMessage as a child type within PureCarousel', () => {
        expect(TestUtils.isElementOfType(result.props.children[0], StructuredMessage)).toEqual(true)
      })
      ...
      it('should accept the correct schema for each children', () => {
        ...
        scenarios.forEach((scenario, index) => {
          ...
          for (const key in scenario) {
            expect(child.props[key]).toEqual(scenario[key])
          }
        })
      })
    })
