describe('ChatGroup component', () => {
  ...
  describe('#renderName', () => {
    let renderName, isAgent, showAvatar, messages, result
  ...
  describe('#renderMessageBubble', () => {
    let result, componentProps, chat, isAgent, showAvatar, handleSendMsgSpy
    ...
    describe('when called', () => {
      ...
      it('returns a MessageBubble component', () => {
        expect(TestUtils.isElementOfType(result, MessageBubble)).toEqual(true)
      })

      it('passes the message value', () => {
        expect(result.props.message).toEqual(chat.msg)
      })

      it('passes the options value', () => {
        expect(result.props.options).toEqual(chat.options)
      })

      it('passes the translatedMessage', () => {
        expect(result.props.translatedMessage).toEqual(chat.translation.msg)
      })
  ...
  describe('#renderErrorMessage', () => {
    let result, component, componentArgs, chat, isAgent, showAvatar, handleSendMsgSpy
  ...
  describe('#renderDefaultMessage', () => {
    let result, component, componentArgs, chat, isAgent, showAvatar
    ...
    describe('when the chat status is not successful', () => {
      ...
      it('does not render an Icon component', () => {
        ...
        expect(TestUtils.isElementOfType(firstChild, Icon)).toEqual(false)
      })
    })
  })

  describe('#renderPrintedMessage', () => {
    let component, chat, isAgent, showAvatar
    ...
    describe('when the chat status has failed to sent', () => {
      ...
      it('calls renderErrorMessage with expected args', () => {
        expect(component.renderErrorMessage).toHaveBeenCalledWith(chat, isAgent, showAvatar)
      })

      it('does not call renderDefaultMessage', () => {
        expect(component.renderDefaultMessage).not.toHaveBeenCalled()
      })
    })
    ...
    describe('when the chat status has not failed to sent', () => {
      ...
      it('calls renderDefaultMessage with expected args', () => {
        expect(component.renderDefaultMessage).toHaveBeenCalledWith(chat, isAgent, showAvatar)
      })

      it('does not call renderErrorMessage', () => {
        expect(component.renderErrorMessage).not.toHaveBeenCalled()
      })
    })
  })

  describe('#renderChatMessages', () => {
    let component, componentArgs, result, isAgent, showAvatar, messages
    ...
    describe('when messages contain a chat.msg type', () => {
      ...
      it('calls renderPrintMessage once with expected args', () => {
        ...
        expect(component.renderPrintedMessage).toHaveBeenCalledWith(chatEvent, isAgent, showAvatar)
      })

      it('does not call renderInlineAttachment', () => {
        expect(component.renderInlineAttachment).not.toHaveBeenCalledWith()
      })
    })

    const structuredMessageTypes = _.values(
    ...
    structuredMessageTypes.forEach(structuredMessageType => {
      describe(`when messages contain a chat.msg with ${structuredMessageType} structured message`, () => {
        ...
        it('calls renderStructuredMessage once with expected args', () => {
          ...
          expect(component.renderStructuredMessage.calls.count()).toEqual(1)

          expect(component.renderStructuredMessage).toHaveBeenCalledWith(chatEvent.structured_msg)
        })

        it('does not call renderPrintedMessage', () => {
          expect(component.renderPrintedMessage).not.toHaveBeenCalledWith()
        })

        it('append the structuredMessageContainer class ', () => {
          expect(result[0].props.children.props.className).toContain(
            'structuredMessageContainerClass'
          )
        })
      })
    })
    ...
    carouselTypes.forEach(carouselType => {
      describe(`when messages contain a chat.msg with ${carouselType} structured message`, () => {
        ...
        it('calls renderCarousel once', () => {
          expect(component.renderCarousel.calls.count()).toEqual(1)
        })

        it('calls renderCarousel with expected args', () => {
          const chatEvent = messages[0]

          expect(component.renderCarousel).toHaveBeenCalledWith(chatEvent.structured_msg.items)
        })
      })
    })
    ...
    describe('when messages contain a chat.file type', () => {
      ...
      it('calls renderInlineAttachment once with expected args', () => {
        ...
        expect(component.renderInlineAttachment.calls.count()).toEqual(1)

        expect(component.renderInlineAttachment).toHaveBeenCalledWith(isAgent, fileEvent)
      })
  ...
  describe('#renderInlineAttachment', () => {
    let renderInlineAttachment, isAgent, chat, result
      ...
      describe('when the attachment filetype is an image', () => {
        ...
        describe('when the file has finished uploading', () => {
          ...
          it('renders the component with an Attachment as the placeholder element', () => {
            expect(TestUtils.isElementOfType(result.props.placeholderEl, Attachment)).toEqual(true)
          })
        })
      })
    })
    ...
    describe('when the showAvatar prop is true', () => {
      describe('and the sender is an agent', () => {
        ...
        it('returns an avatar', () => {
          expect(TestUtils.isElementOfType(result, Avatar)).toBeTruthy()
        })

        it('contains the right classnames', () => {
          expect(result.props.className).toContain('agentAvatarClass')

          expect(result.props.className).not.toContain('endUserAvatarClass')
        })
      })
      ...
      describe('and the sender is an end-user', () => {
        ...
        it('contains the right classnames', () => {
          expect(result.props.className).toContain('endUserAvatarClass')

          expect(result.props.className).not.toContain('agentAvatarClass')
        })
      })
    })
  })
  ...
  describe('#renderStructuredMessage', () => {
    ...
    describe('when called', () => {
      ...
      it('returns a StructuredMessage component', () => {
        expect(TestUtils.isElementOfType(result, StructuredMessage)).toEqual(true)
      })

      it('passes the schema value', () => {
        expect(result.props.schema).toEqual(schema)
      })
    })
  })
