describe('ChatContactDetailsPopup component', () => {
  ...
  beforeEach(() => {
    ...
    EDIT_CONTACT_DETAILS_LOADING_SCREEN = requireUncached(chatConstantsPath)
      .EDIT_CONTACT_DETAILS_LOADING_SCREEN
    EDIT_CONTACT_DETAILS_ERROR_SCREEN = requireUncached(chatConstantsPath)
      .EDIT_CONTACT_DETAILS_ERROR_SCREEN
  ...
  describe('handleSave', () => {
    let component, mockProps, rightCtaFnSpy
    ...
    describe('when form is valid', () => {
      beforeAll(() => {
        mockProps = {
          contactDetails: { display_name: 'bob', email: 'bob@zd.com' }
        }
        mockEmailValid = true
      })

      it('calls props.rightCtaFn with form state name and email', () => {
        expect(rightCtaFnSpy).toHaveBeenCalledWith('bob', 'bob@zd.com')
      })
    ...
    describe('when the user presses <Enter>', () => {
      ...
      describe('when shift _is_ pressed simultaneously', () => {
        it('does not send the message and enters a line break', () => {
          ...
          expect(component.handleSave).not.toHaveBeenCalled()
        })
      })
    })
  ...
  describe('handleFormChange', () => {
    let component, mockProps

    beforeEach(() => {
      updateFnSpy = jasmine.createSpy('updateFn')

      component = instanceRender(
        <ChatContactDetailsPopup updateFn={updateFnSpy} contactDetails={mockProps} />
      )
    ...
    describe('when form is valid', () => {
      ...
      it('updates state.showEmailError as false via emailValid', () => {
        expect(component.state.showEmailError).toBe(false)
      })

      it('attempt to update redux with new target name field', () => {
        expect(updateFnSpy).toHaveBeenCalledWith('bob', 'hello@world.com')
      })
    })
    ...
    describe('when form is invalid', () => {
      ...
      it('attempt to update redux with new target email field', () => {
        expect(updateFnSpy).toHaveBeenCalledWith('Joey', 'bob')
      })
    })
  })

  describe('componentWillReceiveProps', () => {
    let component, mockContactDetails
    ...
    describe('when contact details have been entered into the popup before', () => {
      beforeEach(() => {
        ...
        component = instanceRender(
          <ChatContactDetailsPopup contactDetails={mockContactDetails} updateFn={updateFnSpy} />
        )
      ...
      it('attempts to update the contactDetails redux with the input values', () => {
        expect(updateFnSpy).toHaveBeenCalledWith('guy', 'guy@guys.com')
      })
    })
    ...
    describe('when contact details have not been entered into the popup before', () => {
      ...
      describe('when the name is not the default chat name', () => {
        it('attempts to update the contactDetails with the visitor prop', () => {
          expect(updateFnSpy).toHaveBeenCalledWith('bob', 'bob@bob.com')
        })
      })
      ...
      describe('when the name is the default chat name', () => {
        beforeEach(() => {
          ...
          component.componentWillReceiveProps({
            visitor: { display_name: 'Visitor 12345' },
            contactDetails: {}
          })
        })

        it('sets name in contactDetails redux to an empty string', () => {
          expect(updateFnSpy).toHaveBeenCalledWith('', '')
        })
      })
  ...
  describe('render', () => {
    ...
    describe('when method is called', () => {
      ...
      it('calls renderForm', () => {
        expect(component.renderForm).toHaveBeenCalled()
      })
  ...
  describe('renderLoadingSpinner', () => {
    ...
    describe('when the state is a loading screen', () => {
      ...
      it('renders a loading spinner component', () => {
        expect(TestUtils.isElementOfType(loadingSpinner, LoadingSpinner)).toEqual(true)
      })

      it('passes the right width prop', () => {
        expect(loadingSpinner.props.width).toEqual(32)
      })

      it('passes the right height prop', () => {
        expect(loadingSpinner.props.height).toEqual(32)
      })
    })
  })
