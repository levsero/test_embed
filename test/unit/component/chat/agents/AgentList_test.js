  ...
  describe('render', () => {
    ...
    describe('when agents are passed to the prop', () => {
      ...
      it('returns an element for each agent', () => {
        expect(component.render().length).toEqual(Object.keys(agents).length)
      })
    })
    ...
    describe('avatar', () => {
      ...
      beforeEach(() => {
        avatar = findComponentByType(component.render()[0].props.children, Avatar)
      })

      it('is rendered with the src set to the avatar_path', () => {
        expect(avatar.props.src).toEqual(agent.avatar_path)
      })

      it('is rendered with the fallbackIcon set to Icon--agent-avatar', () => {
        expect(avatar.props.fallbackIcon).toEqual('Icon--agent-avatar')
      })
    })
  })
})
