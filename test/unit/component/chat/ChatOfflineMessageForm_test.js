describe('ChatOfflineMessageForm component', () => {
  ...
  describe('render', () => {
    ...
    it('renders the name value from the form to the screen', () => {
      expect(formResults[0].props.children).toBe(mockFormValues.name)
    })

    it('renders the email value from the form to the screen', () => {
      expect(formResults[1].props.children).toBe(mockFormValues.email)
    })

    it('renders the phone value from the form to the screen', () => {
      expect(formResults[2].props.children).toBe(mockFormValues.phone)
    })

    it('renders the message value from the form to the screen', () => {
      expect(formResults[3].props.children).toBe(mockFormValues.message)
    })
  })
})
