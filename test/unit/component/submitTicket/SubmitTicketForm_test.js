describe('SubmitTicketForm component', () => {
    ...
    mockRegistry = initMockRegistry({
      ...
      'component/button/ButtonSecondary': {
        ButtonSecondary: class extends Component {
          render() {
            return <div className="c-btn--secondary" label="Cancel" onClick={onCancel} />
          }
        }
      },
  ...
  it("displays the form's title", () => {
    ...
    expect(document.getElementById('formTitle').innerHTML).toEqual('a lovely title')
  })

  it('correctly renders form with noValidate attribute', () => {
    const submitTicketForm = domRender(<SubmitTicketForm />)

    expect(ReactDOM.findDOMNode(submitTicketForm).getAttribute('novalidate')).toEqual('')
  })
  ...
  describe('submit button', () => {
    ...
    it('is not disabled by default', () => {
      expect(submitElem.disabled).toEqual(false)
    })
    ...
    describe('when canSubmit is false', () => {
      ...
      it('is disabled', () => {
        expect(submitElem.disabled).toEqual(true)
      })
    })
  ...
  describe('on a valid submission', () => {
    ...
    it('clears all fields other then name and email', () => {
      expect(mockFormState).toEqual(expectedFormState)
    })

    it('calls prefillFormState', () => {
      expect(submitTicketForm.prefillFormState).toHaveBeenCalled()
    })

    it("resets the button to it's initial state", () => {
      expect(submitTicketForm.state.buttonMessage).toEqual(
    ...
    describe('when attachments are enabled', () => {
      ...
      it('clears the attachments list', () => {
        expect(mockAttachmentsListClear).toHaveBeenCalled()
      })
    })
  })
  ...
  describe('clearAttachments', () => {
    ...
    beforeEach(() => {
      const submitTicketForm = domRender(
        <SubmitTicketForm attachmentsEnabled={attachmentsEnabled} />
      )
    ...
    describe('when attachments are not enabled', () => {
      ...
      it('does not call attachment list clear', () => {
        expect(mockAttachmentsListClear).not.toHaveBeenCalled()
      })
    })
  })
  ...
  describe('#renderSubjectField', () => {
    ...
    describe('when the subject field is enabled', () => {
      ...
      beforeEach(() => {
        const submitTicketForm = instanceRender(<SubmitTicketForm subjectEnabled={true} />)
      ...
      it('renders the subject field with correct label', () => {
        expect(renderLabelSpy).toHaveBeenCalledWith(
          Label,
          'embeddable_framework.submitTicket.field.subject.label',
          false
        )
      })

      it('renders the subject field with correct name', () => {
        expect(inputElement.props.name).toBe('subject')
      })
    })
  ...
  describe('#renderEmailField', () => {
    ...
    it('renders the email field with correct name prop', () => {
      expect(inputElement.props.name).toBe('email')
    })
  })
  ...
  describe('#renderDescriptionField', () => {
    ...
    it('renders the description field with correct name prop', () => {
      expect(inputElement.props.name).toBe('description')
    })
  })
  ...
  describe('When a Ticket Form is passed in', () => {
    ...
    beforeEach(() => {
      ...
      component = domRender(
        <SubmitTicketForm activeTicketForm={mockTicketForm} ticketFields={mockTicketFields} />
      )
    ...
    it('renders the name field', () => {
      expect(renderLabelSpy).toHaveBeenCalledWith(
        Label,
        'embeddable_framework.submitTicket.field.name.label',
        false
      )
    })
    ...
    it('renders the email field', () => {
      expect(renderLabelSpy).toHaveBeenCalledWith(
        Label,
        'embeddable_framework.form.field.email.label',
        true
      )
    })

    it('renders the extra fields defined in the ticket form', () => {
      expect(formElements[3].getAttribute('name')).toBe('1')
      expect(formElements[4].getAttribute('name')).toBe('2')
      expect(formElements[5].getAttribute('name')).toBe('4')
    })

    it('does not render any fields not defined in the ticket form', () => {
      expect(formElements.length).not.toBe(4)
    })
  })
  ...
  describe('handleSubmit', () => {
    ...
    describe('when the preview is enabled', () => {
      ...
      it('does not change the button label', () => {
        expect(submitTicketForm.state.buttonMessage).toBe(
          'embeddable_framework.submitTicket.form.submitButton.label.send'
        )

        expect(submitTicketForm.state.isSubmitting).toBe(false)
      })
    ...
    describe('when the form is invalid', () => {
      ...
      it('sets showErrors to true', () => {
        expect(submitTicketForm.state.showErrors).toEqual(true)
      })
    })
  })
  ...
  describe('renderErrorMessage', () => {
    ...
    describe('when shouldRenderErrorMessages is true', () => {
      ...
      it('returns the garden error message component', () => {
        expect(TestUtils.isElementOfType(error, Message)).toEqual(true)
      })
    })
  ...
  describe('isPrefillValid', () => {
    ...
    describe('when given a value that is not an array of items', () => {
      it('returns false', () => {
        ...
        subjects.forEach(subject => {
          expect(submitTicketForm.isPrefillValid(subject)).toEqual(false)
        })
      })
    })
    ...
    describe('when given an array of items', () => {
      it('returns true', () => {
        expect(submitTicketForm.isPrefillValid([1, 2, 3])).toEqual(true)
      })
    })
  })
  ...
  describe('mergePrefill', () => {
    ...
    describe('when both pre-fill form and field is given', () => {
      ...
      describe('with existing ids in both data sets', () => {
        it('does not overwrite pre-fill field data', () => {
          ...
          expect(result).toEqual(mockPrefillTicketForm)
        })
      })
    ...
    describe('when only pre-fill field data is given', () => {
      it('returns pre-fill field data', () => {
        ...
        expect(result).toEqual(mockPrefillFields)
      })
    })
    ...
    describe('when pre-fill contains disallowed field types', () => {
      const mockPrefillTicketForm = {
        fields: [
          ...
          {
            id: 'email',
            prefill: { '*': 'def', 'en-GB': 'arycelle@dania.com' }
          },
          { id: '4444444', prefill: { '*': 'ghi', 'en-GB': 'Boco' } },
          { id: 5555555, prefill: { '*': 'jkl', 'en-GB': 'Luso' } },
          {
            id: 1234567,
            prefill: { '*': 'field should not', 'en-GB': 'exist' }
          }
        ]
      }
  ...
  describe('prefillFormState', () => {
    let submitTicketForm, mockSetFormState
    ...
    describe('when pre-fill contains allowed field types', () => {
      ...
      describe("when current locale is 'en-GB'", () => {
        ...
        it('pre-fills ticket fields in the ticket form', () => {
          ...
          expect(mockSetFormState.calls.mostRecent().args).toEqual(expectation)
        })
      })
      ...
      describe('when the fields do not contain description', () => {
        ...
        it('still pre-fills description if it is passed in', () => {
          ...
          expect(mockSetFormState.calls.mostRecent().args[0]).toEqual(
            jasmine.objectContaining({ description: 'my prefill' })
          )
        })
      })
    })
