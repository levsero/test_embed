...
describe('getBuid', () => {
  ...
  it('sets a new buid if none is available', () => {
    ...
    expect(store.get('buid')).toEqual(buid)
  })
})
...
describe('getSuid', () => {
  ...
  it('returns the current suid if it already exists and is not expired', () => {
    ...
    expect(newSuid).toEqual(oldSuid)
  })
  ...
  it('generates a new suid if the current one is expired', () => {
    ...
    expect(suid.id).not.toEqual(1234)
...
describe('unload', () => {
  ...
  it('decrements tab count of suid', () => {
    ...
    expect(suid.tabs.expiry).not.toEqual(5)
    expect(suid).toEqual(
      expect.objectContaining({
        id: 1,
        expiry: 1234,
