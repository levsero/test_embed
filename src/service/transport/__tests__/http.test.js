...
describe('#send', () => {
  ...
  it('sets the correct http method and url on superagent', () => {
    ...
    expect(superagent).toHaveBeenCalledWith('GET', 'https://test.zendesk.host/test/path')
  })
  ...
  describe('when not forcing http', () => {
    ...
    it('uses the protocol from the config', () => {
      expect(urlArg).toEqual(expect.stringContaining(config.scheme))
    })

    it('uses the zendesk domain from the config', () => {
      expect(urlArg).toEqual(expect.stringContaining(config.zendeskHost))
    })
  })
  ...
  describe('when forcing http', () => {
    ...
    it('uses the http protocol', () => {
      expect(urlArg).toEqual(expect.stringContaining('http:'))
    })
  })
...
describe('#getImage', () => {
  ...
  it('sets the correct http method and url on superagent', () => {
    expect(superagent).toHaveBeenCalledWith('get', payload.path)
  })

  it('sets the responseType to `blob`', () => {
    expect(superagent.__mostRecent().responseType).toHaveBeenCalledWith('blob')
  })

  it('sets an authentication header with `Bearer <token>`', () => {
    expect(superagent.__mostRecent().set).toHaveBeenCalledWith(
      'Authorization',
  ...
  describe('when there is no error', () => {
    ...
    it('invokes the done callback', () => {
      expect(payload.callbacks.done).toHaveBeenCalledWith(res)
    })
  })
  ...
  describe('when there is an error', () => {
    ...
    it('invokes the fail callback', () => {
      expect(payload.callbacks.fail).toHaveBeenCalledWith(err, null)
    })
  })
})
...
describe('#sendFile', () => {
  ...
  describe('when zendeskHost is not set in config', () => {
    ...
    it('throws an exception', () => {
      expect(() => http.send(payload)).toThrow()
    })
  })
