...
describe('connect', () => {
  ...
  it('calls io with the correct service url', () => {
    expect(args[0]).toBe(`https://${mockTalkServiceHostname}`)
  })
  ...
  it('returns the socket', () => {
    expect(socket).toBe('mockReturnValue')
  })
})
...
describe('mapEventsToActions', () => {
  ...
  it('calls talkEmbeddableConfigEventToAction with socket and reduxStore', () => {
    expect(talkEmbeddableConfigEventToAction).toHaveBeenCalledWith(socket, reduxStore)
  })

  it('calls talkAgentAvailabilityEventToAction with socket and reduxStore', () => {
    expect(talkAgentAvailabilityEventToAction).toHaveBeenCalledWith(socket, reduxStore)
  })

  it('calls talkAverageWaitTimeEventToAction with socket and reduxStore', () => {
    expect(talkAverageWaitTimeEventToAction).toHaveBeenCalledWith(socket, reduxStore)
  })
})
