...
describe('getSessions', () => {
  it('returns the sessions', () => {
    ...
    expect(selectors.getSessions({ answerBot: { sessions } }).size)
      .toEqual(2);
  });
});
...
describe('isInitialSession', () => {
  describe('1 active session', () => {
    it('returns true', () => {
      ...
      expect(selectors.isInitialSession.resultFunc(sessions))
        .toEqual(true);
    });
  });
...
describe('getSessionFallbackSuggested', () => {
  it('returns the session fallbackSuggested', () => {
    ...
    expect(selectors.getSessionFallbackSuggested.resultFunc(1, sessions))
      .toBe(true);
    expect(selectors.getSessionFallbackSuggested.resultFunc(2, sessions))
      .toBe(false);
  });
});
