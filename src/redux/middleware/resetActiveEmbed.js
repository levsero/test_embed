...
export default function resetActiveEmbed(prevState, nextState, action, dispatch = () => {}) {
  ...
  const shouldResetForChatChannelChoice = type === CLOSE_BUTTON_CLICKED
    && !getChatAvailable(prevState)
    && getActiveEmbed(prevState) === 'channelChoice';

  if ((!widgetVisible && shouldReset) || shouldResetForSuppress(action, prevState) || shouldResetForChatChannelChoice) {
