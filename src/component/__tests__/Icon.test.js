  ...
  describe('keydown event', () => {
    const simulateKeyDown = keyEvent => {
      ...
      const { container } = render(
        <IconButton onClick={handler} altText="Clickable Icon" type="Icon--zendesk" />
      )
      const button = getButton(container)
    ...
    it('fires the onclick handler on Enter', () => {
      expect(simulateKeyDown({ key: 'Enter', keyCode: 13 })).toHaveBeenCalled()
    })

    it('does not fire the onclick handler on other key codes', () => {
      expect(simulateKeyDown({ key: 'Space', keyCode: 32 })).not.toHaveBeenCalled()
    })
  })
})
