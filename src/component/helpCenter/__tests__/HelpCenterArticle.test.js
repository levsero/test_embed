...
describe('clicking article links', () => {
  ...
  test('displays an empty article body if a prop was passed with no content body', () => {
    ...
    expect(container.querySelector('.article').innerHTML)
      .toEqual('');
  });
});
  ...
  describe('valid urls', () => {
    [
      ...
      test(`${src} gets the expected iframe tag with stripped attrs`, () => {
        ...
        expect(iframe.outerHTML).toEqual(
...
test('does not render original article button if it renderOriginalArticleButton is false', () => {
  const { queryByTitle } = renderComponent({ originalArticleButton: false, activeArticle });

  expect(queryByTitle('View original article'))
    .not.toBeInTheDocument();
});
...
describe('article images', () => {
  it('does not inject the locale in the image url if not authenticated', () => {
    ...
    expect(getByAltText('img').src)
      .toEqual('https://dev.zd-dev.com/hc/article_attachments/img1.png');
  });
  ...
  describe('baseIsAuthenticated', () => {
    it('queues up new images with locale in the image url', () => {
      ...
      renderComponent({ imagesSender, updateStoredImages, activeArticle: article });
      expect(imagesSender).toHaveBeenCalledWith('https://dev.zd-dev.com/hc/en-us/article_attachments/img1.png', expect.any(Function));
      expect(imagesSender).toHaveBeenCalledWith('https://dev.zd-dev.com/hc/en-us/article_attachments/img2.png', expect.any(Function));
  ...
  it('returns the article with image', () => {
    ...
    expect(getByAltText('img').src)
      .toEqual('https://cdn.com/id/img.png');
  });
  ...
  it('injects the host name when there are images in the article with relative `/attachments/` paths', () => {
    ...
    expect(getByAltText('img').src)
      .toContain('//localhost/attachments/token/abc/?name=img.png');
  });
});
