...
describe('with results', () => {
  ...
  test('renders expected heading when hasContextualSearched is true', () => {
    const { queryByText } = renderComponent({ hasContextualSearched: true, articles });

    expect(queryByText('Top suggestions'))
      .toBeInTheDocument();
  });

  test('renders expected heading when hasContextualSearched is false', () => {
    const { queryByText } = renderComponent({ hasContextualSearched: false, articles });

    expect(queryByText('Top results'))
      .toBeInTheDocument();
  });

  describe('listBottom style', () => {
    test('contains style if showContactButton is true', () => {
      const { container } = renderComponent({ showContactButton: true, articles });

      expect(container.querySelector('ol'))
        .toHaveClass('listBottom');
    });

    test('contains style if hideZendeskLogo is true and showContactButton is false', () => {
      const { container } = renderComponent({ hideZendeskLogo: true, showContactButton: false, articles });
    ...
    test('does not contain style if hideZendeskLogo and showContactButton are false', () => {
      const { container } = renderComponent({ hideZendeskLogo: false, showContactButton: false, articles });

      expect(container.querySelector('ol'))
        .not.toHaveClass('listBottom');
    });
  });
});
...
describe('messaging', () => {
  test('has contextual searched and contextual search is complete', () => {
    ...
    expect(queryByText('Enter a term in the search bar above to find articles.'))
      .toBeInTheDocument();
  });
  ...
  test('has not contextual searched', () => {
    ...
    expect(queryByText('There are no results for ""'))
      .toBeInTheDocument();
    expect(queryByText('Try searching for something else.'))
      .toBeInTheDocument();
  });
  ...
  test('search has not failed and previous search term is present', () => {
    ...
    expect(queryByText('There are no results for "Help me!"'))
      .toBeInTheDocument();
    expect(queryByText('Try searching for something else.'))
      .toBeInTheDocument();
  });
  ...
  test('search has failed and showing contact button', () => {
    ...
    expect(queryByText('Sorry, there are no results.'))
      .toBeInTheDocument();
    expect(queryByText('Click the button below to send us a message.'))
      .toBeInTheDocument();
  });
