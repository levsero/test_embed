...
- Web SDK launch. For more information, see [Introducing Zendesk Messaging](https://support.zendesk.com/hc/en-us/articles/360057455393-Introducing-Zendesk-Messaging)

### 3 February 2021

- Fixed a bug where an offline message is sent to a default department instead of requesting the user to
    select a valid department. This occurred when:
  - A chat trigger set the default department to an offline department
  - The offline department is filtered out of the pre-chat form in the API settings
  - The user sends a message with the offline form without selecting the department
...
- Fixed small bug causing the Web Widget to crash if no `prefill` option is passed to the ticket forms fields API
...
- Fixed inconsistencies in the margins used in Web Widget positioning. Margins will now default to 16px on each side
...
- Fixed a bug that prevented Philippine phone numbers from rendering in the Web Widget
...
- Updated the Web Widget to use a more generic and consistent success page when the
...
- Fixed a bug that caused the Web Widget to not be hidden when the `hide` API command
  was called straight after the `show` API command on page load
- Fixed multi-line label spacing in contact forms

### 14 May 2020

- Added improvements in the Web Widget loading time by deferring when certain data is loaded and processed
...
- Changed Answer Bot to now only download on-demand instead of when the Web Widget starts on page load
...
- Fixed a bug where the Chat badge did not display the Web Widget launcher after the user had interacted with it
...
- Fixed minor visual bug by removing spacing near the Web Widget footer section
...
- Fixed a bug where de-activated ticket forms took a very long time to stop showing up in the Web Widget
- Fixed a bug in the ticket submission form where user entries were lost between opening and closing the form

### 16 March 2020

- Fixed the Answer Bot greeting being duplicated when the Web Widget is opened by the `zE('webWidget', 'open')` API
...
- Fixed a bug that was causing support forms to crash when it included a field the Web Widget does not support
...
- Fixed Chat menu visibility when viewing the Web Widget in a pop-out window
...
- Changed Help Center to load only when users need it, saving end user data and reducing overall Web Widget loading time
...
- [Translation optimizations](https://supportzendesk.com/hc/en-us/articles/360019791353-Web-Widget-Performance-Improvement-Optimized-Translations) was released
