describe('chat reducer accountSettings attachment', () => {
  ...
  describe('reducer', () => {
    ...
    describe('initial state', () => {
      it('enabled is true', () => {
        expect(initialState.enabled).toEqual(true)
      })

      it('allowed_extensions is an array containing some extensions', () => {
        expect(initialState.allowed_extensions).toEqual('png,jpg,jpeg,gif,txt,pdf')
      })
    })
    ...
    describe('when a GET_ACCOUNT_SETTINGS_REQUEST_SUCCESS action is dispatched', () => {
      ...
      it('sets the action payload as the state', () => {
        expect(state).toEqual(settings.file_sending)
      })
    })
