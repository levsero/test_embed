...
describe('when ADD_TO_AFTER_SHOW_ANIMATE is dispatched', () => {
  it('adds the payload to the array', () => {
    expect(reduce(actionTypes.ADD_TO_AFTER_SHOW_ANIMATE, mockCallback)).toEqual(
      expect.arrayContaining([mockCallback])
    )
  })

  it('does not override anything already in the array', () => {
    expect(reduce(actionTypes.ADD_TO_AFTER_SHOW_ANIMATE, mockCallback, [mockCallback])).toEqual(
      expect.arrayContaining([mockCallback, mockCallback])
    )
  })
})
...
describe('when WIDGET_SHOW_ANIMATION_COMPLETE is dispatched', () => {
  it('resets the state to the initial state', () => {
    expect(reduce(actionTypes.WIDGET_SHOW_ANIMATION_COMPLETE, mockCallback, [() => {}])).toEqual(
      initialState()
    )
  })
})
